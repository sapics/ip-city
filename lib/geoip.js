...
function setCityRecord(buffer, geodata, offset, locBuffer){
	...
	if(-1 >>> locId){
		...
		var sub1 = locBuffer.readUInt16BE(locOffset, locOffset += 2);
		if(sub1 > 0) {
			geodata.region1 = region1Json[sub1];
			geodata.state1 = state1Json[sub1];
		}
		var sub2 = locBuffer.readUInt16BE(locOffset, locOffset += 2);
		if(sub2 > 0) {
			geodata.region2 = region2Json[sub2];
			geodata.state2 = state2Json[sub2];
		}
		var metro = locBuffer.readUInt16BE(locOffset, locOffset += 2);
		if(metro > 0) geodata.metro = metro;
		var city = locBuffer.readUInt32BE(locOffset, locOffset += 4);
		if(city > 0) geodata.city = cityJson[city];
		var timezone = locBuffer.readUInt16BE(locOffset, locOffset += 2);
